  ...
  public:
    void find(Node* root, Node* &prev, Node* &first, Node* &second)
    {
        ...
        if(prev)
        {
            ...
            {
                //first decrease - both first and second are null
                if(!first&&!second)
                {
                    first = prev;
                    second = root;
                }
                //second decrease - both first and second have some value
                else
                {
                    second = root;
                }
        ...
        prev = root;//storing last value
        //right traverse
        find(root->right, prev, first, second);
    }
    struct Node *correctBST(struct Node *root) {
        //Logic - in single dip - first and second are adjacent
        //in multi dip - it is later value in 2nd dip, former value in 1st dip
        //dry run example in array form
        //BST - hard lecture
